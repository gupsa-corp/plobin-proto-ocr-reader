...
class DocumentBlockExtractor:
    ...
    def extract_blocks(self, image_path: str, confidence_threshold: float = 0.5, merge_blocks: bool = True, merge_threshold: int = 30) -> Dict:
        """
        이미지에서 문서 블록 추출
        ...
        if merge_blocks and blocks:
            blocks = self._merge_adjacent_blocks(blocks, merge_threshold)
        ...
        for i in range(1, len(sorted_blocks)):
            ...
            if y_diff <= height_avg * 0.5:
                ...
                if len(current_line_blocks) > 1:
                    merged_block = self._merge_line_blocks(current_line_blocks, merge_threshold)
    ...
    def _merge_line_blocks(self, line_blocks: List[Dict], merge_threshold: int) -> Dict:
        """
        같은 줄의 블록들을 하나로 병합
        ...
        for i in range(1, len(sorted_blocks)):
            ...
            if x_distance <= merge_threshold:
                ...
                if len(current_group) > 1:
                    merged_groups.append(self._create_merged_block(current_group))
                else:
        ...
        if len(merged_groups) == 1:
            ...
            return self._create_merged_block(merged_groups)

    def _create_merged_block(self, blocks: List[Dict]) -> Dict:
        """
        여러 블록을 하나로 병합
